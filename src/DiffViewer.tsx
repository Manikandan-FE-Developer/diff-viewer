...
const highlightDiff = (oldLine: string | null, newLine: string | null) => {
    ...
    if (!oldLine && newLine) return [{ text: newLine, type: 'added' }];
    if (oldLine && !newLine) return [{ text: oldLine, type: 'removed' }];
    ...
    for (let i = 0; i < lcs.length; i++) {
        while (oldLine && oldLine[oldIdx] !== lcs[i] && oldIdx < oldLine.length) {
            result.push({ text: oldLine[oldIdx++], type: 'removed' });
        }
        while (newLine && newLine[newIdx] !== lcs[i] && newIdx < newLine.length) {
    ...
    while (oldLine && oldIdx < oldLine.length) {
        result.push({ text: oldLine[oldIdx++], type: 'removed' });
    }
    while (newLine && newIdx < newLine.length) {
        result.push({ text: newLine[newIdx++], type: 'added' });
    }
